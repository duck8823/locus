...
/// - 先頭が大小英字いずれでも可 (`fixes #45`)
/// - 単独の `#123` も抽出
/// - `owner/repo#123` 形式は本関数ではスキップ（cross-repo 参照）
pub fn extract_linked_issue_numbers(body: &str) -> Vec<u64> {
    ...
    while i < bytes.len() {
        if bytes[i] == b'#' {
            // cross-repo は直前が alphanumeric + '/' なのでスキップ
            let is_cross_repo = i > 0 && (bytes[i - 1].is_ascii_alphanumeric() || bytes[i - 1] == b'/');
            if !is_cross_repo {
                let mut j = i + 1;
                ...
                if j > i + 1
                    && let Ok(n) = std::str::from_utf8(&bytes[i + 1..j]).unwrap().parse::<u64>()
                {
                    found.insert(n);
                }
                i = j;
